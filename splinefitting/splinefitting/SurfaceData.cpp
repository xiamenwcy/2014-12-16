...
bool   CSurfaceData::knots_iteration()
{
	...
	int  i=0;
	uknots2=new float[unum];
	vknots2=new float[vnum];
	update_polymesh();  
	while(i<iter_times&&!iter_unusual)
	{
      update_knots(i);
	  update_polymesh();
	  i++;
...
void  CSurfaceData::update_knots(int k)
{
	...
	OpenMesh::FPropHandleT<float>  fr_area;//增加每个面的真实面积
	polymesh.add_property(fr_area,"fr_area");
	...
	OpenMesh::EPropHandleT<vecint> e_index;  //增加每条边的关联面序号
	polymesh.add_property(e_index,"e_index");
    ...
    float esum=0.0f;
	int  num=0;
	vector<vector<int> > uknot_index(unum-2),vknot_index(vnum-2);//分别存储每条节点线的半边序号
	vector<float> uknot_grad(unum-2),vknot_grad(vnum-2);//分别存储每条节点线的梯度
	vector<float> uknot_step(unum-2),vknot_step(vnum-2);//分别存储每条节点线的步长
	...
	for (e_it=polymesh.edges_begin();e_it!=e_end;++e_it)
	{
		if (!polymesh.is_boundary(*e_it))
		{
			...
			for (vfiter=polymesh.vf_iter(fromvertex);vfiter.is_valid();++vfiter)
		 {
			...
			polymesh.property(e_index,*e_it)=inter_index;
          ...
          //分别存储每条节点线的半边序号
		   ...
		   {
			   if (abs(uknots[i]-emid[0])<=EPSINON)
			   {
				   uknot_index[i-1].push_back((*e_it).idx());
			   }

		   }
		   for (int j=1;j<vnum-1;j++)
		   {
			   if (abs(vknots[j]-emid[1])<=EPSINON)
			   {
				   vknot_index[j-1].push_back((*e_it).idx());
			   }
...
//增加每个面的真实面积
	...
	{
		fv.clear();
		for (fv_iter=polymesh.fv_iter(*face_iter);fv_iter.is_valid();++fv_iter)
		{
			test5[0]=(polymesh.point(*fv_iter)).data()[0];
			test5[1]=(polymesh.point(*fv_iter)).data()[1];
			fv.push_back(test5);
		}
        polymesh.property(fr_area,*face_iter)=area(fv);
	}
//节点更新
	//先准备步长与梯度
	vector<vector<int> >::iterator oo;
	vector<int>::iterator  pp;
	MyMesh::EdgeHandle ee;
	int k_iter=0;
	for (oo=uknot_index.begin();oo!=uknot_index.end();++oo)
	{
		for (pp=(*oo).begin();pp!=(*oo).end();++pp)
		{
			inter_index.clear();
			ee=polymesh.edge_handle(*pp);
            inter_index=polymesh.property(e_index,ee);
			ef1=polymesh.face_handle(inter_index[0]);
			ef2=polymesh.face_handle(inter_index[1]);
           uknot_grad[k_iter]+=(polymesh.property(f_area,ef1)-polymesh.property(f_area,ef2))*polymesh.property(e_area,ee);
		   uknot_step[k_iter]+=(polymesh.property(fr_area,ef1)+polymesh.property(fr_area,ef2));

		}
		uknot_step[k_iter]=sqrtf(uknot_step[k_iter])*0.5f;
		uknot_grad[k_iter]*=2;
		k_iter++;
	...
	out_file<<"cns"<<endl;
	k_iter=0;
	for (oo=vknot_index.begin();oo!=vknot_index.end();++oo)
	{
		for (pp=(*oo).begin();pp!=(*oo).end();++pp)
		{
			inter_index.clear();
			...
			ef1=polymesh.face_handle(inter_index[0]);
			ef2=polymesh.face_handle(inter_index[1]);
			vknot_grad[k_iter]+=(polymesh.property(f_area,ef1)-polymesh.property(f_area,ef2))*polymesh.property(e_area,ee);
			vknot_step[k_iter]+=(polymesh.property(fr_area,ef1)+polymesh.property(fr_area,ef2));

		}
		vknot_step[k_iter]=sqrtf(vknot_step[k_iter])*0.5f;
		vknot_grad[k_iter]*=2;
		k_iter++;
...
{
	uknots[i]-=uknot_step[i-1]*powf(0.5,k/(iter_times-k))*uknot_grad[i-1]/abs(uknot_grad[i-1]);

}
for (int j=1;j<=vnum-2;j++)
{
	vknots[j]-=vknot_step[j-1]*powf(0.5,k/(iter_times-k))*vknot_grad[j-1]/abs(vknot_grad[j-1]);

}
//越界处理
float umin_ele=*(min_element(uknots+1,uknots+unum-1));
float umax_ele=*(max_element(uknots+1,uknots+unum-1));
float vmin_ele=*(min_element(vknots+1,vknots+vnum-1));
float vmax_ele=*(max_element(vknots+1,vknots+vnum-1));
if (umin_ele<0.0||umax_ele>1.0||vmin_ele<0.0||vmax_ele>1.0)
{
...
sort(uknots,uknots+unum);
sort(vknots,vknots+vnum);
